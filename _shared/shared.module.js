...
if (location.protocol === "file:") {
  alert(
    "ERROR: You must run examples on a webserver (not using the file: protocol)"
  );
}
...
const hintKey = "preventhints-" + document.location.href,
  ...
  isDemoBrowser = browserPaths.some(
    (path) =>
      pathName.endsWith(path) || Boolean(pathName.match(path + "index.*html$"))
  ),
  isOnline = /^(www\.)?bryntum\.com/.test(location.host),
  moduleTag = document.querySelector("script[type=module]"),
  isModule =
    pathName.endsWith("module.html") ||
    (moduleTag && moduleTag.src.includes("app.module.js")) ||
    (pathName.endsWith("index.html") && isOnline),
...
class Shared extends Localizable(Events()) {
  ...
  constructor() {
    ...
    me.localeManager.on("locale", (localeConfig) => {
      BrowserHelper.setLocalStorageItem(
        "b-example-language",
        localeConfig.locale.localeName
      );
    ...
    me.localeManager.applyLocale(
      BrowserHelper.getLocalStorageItem("b-example-language") ||
  ...
  get currentThemeId() {
    const themeId =
      BrowserHelper.queryString.theme ||
      BrowserHelper.getLocalStorageItem("b-example-theme") ||
      defaultTheme;
  ...
  insertHeader() {
    const pathElements = document.location.pathname
        .split("/")
        .reduce((result, value) => {
    ...
    if (!header) {
      header = DomHelper.insertFirst(document.getElementById("header"), {
        tag: "header",
    ...
    header.innerHTML = `
            <div id="title-container">
                <a id="title" href="${isDemoBrowser ? "#" : "../../examples/"}${
      !isDemoBrowser && isUmd ? "index.umd.html" : ""
    }#example-examples-scheduler-${exampleId}">
                    ${exampleName}
                </a>
            </div>
            <div id="tools"></div>
  ...
  loadDescription() {
    ...
    AjaxHelper.get(url, { parseJson: true }).then((response) => {
      ...
      let localeValue = me.localeManager.locale.localeName,
        storedLocaleValue = BrowserHelper.getLocalStorageItem(
          "b-example-language"
        ),
        themeCombo;
      ...
      button.menuItems = [
        ...
        .concat(
          themeInfo && themes[themeInfo.name.toLowerCase()]
            ? [
                (themeCombo = {
                  type: "combo",
                  ref: "themeCombo",
            ...
            : [
                {
                  ...
                  onAction: ({ value }) => {
                    ...
                    setTimeout(
                      () => document.body.classList.remove("b-change-size"),
                      400
                    );
                    me.curSize = value;
                    button.menu.hide();
  ...
  initRootCause() {
    const recordVideo = queryString.video === "1",
      disabled =
        !recordVideo &&
        disableRootCause.some((exclude) => queryString[exclude]),
      isRootCauseReplay = (() => {
        try {
  ...
  startRootCause() {
    if ("bugbash" in queryString) {
      ...
      document.cookie =
        "bugbash=1" + "; expires=" + date.toUTCString() + "; path=/";
    }
    ...
    window.logger = new RC.Logger({
      ...
      onBeforeLog(data) {
        ...
        if (
          data.isJsError &&
          (!data.file ||
            !data.file.includes("." + "js") ||
            data.file.includes("chrome-extension"))
        ) {
          return false;
        }
      },
  ...
  performVersionCheck() {
    const lastCheck = BrowserHelper.getLocalStorageItem(
        "b-latest-version-check-timestamp"
      ),
      checkProductName = `${this.productName}${
        this.productName.toLowerCase().match(/schedulerpro|grid/)
          ? ""
          : "-vanilla"
      }`;
    ...
    if (
      lastCheck &&
      Date.now() - new Date(Number(lastCheck)) < 1000 * 60 * 60 * 24 * 2
    ) {
      return;
    }
    ...
    AjaxHelper.get(`https://bryntum.com/latest/?product=${checkProductName}`, {
      ...
      .then(this.notifyIfLaterVersionExists)
      .catch(() => {});
  ...
  notifyIfLaterVersionExists(response) {
    ...
    if (
      latestVersion &&
      VersionHelper.checkVersion(productName, latestVersion, "<")
    ) {
      ...
      toast.element.addEventListener("click", () => {
        const inTwoWeeks = new Date().setDate(new Date().getDate() + 14);
...
class CodeEditor extends Popup {
  ...
  updateCSS() {
    ...
    me.codeCache[me.filename] = me.cssElement.innerHTML =
      me.codeElement.innerText;
  }
  ...
  async updateJS() {
    const me = this,
      ...
      renderedElements = new Set(
        document.querySelectorAll("[data-app-element=true]")
      ),
    ...
    Array.from(me.codeElement.querySelectorAll("pre [style]")).forEach(
      (el) => (el.style = "")
    );
    ...
    try {
        ...
        imports.forEach((imp) => {
          ...
          if (parts.length) {
            const // ../_shared needs Grid/examples, while ../../lib needs Grid/
              absolute = pathParts
                .slice()
                .splice(0, pathParts.length - parts.length)
                .join("/"),
              // import xx from 'http://lh/Grid/lib...'
              statement = `${parts[0]}${base}${absolute}/${
      ...
      renderedWidgets.forEach(
        (widget) => !widget.isDestroyed && !widget.keep && widget.destroy()
      );
  ...
  async loadCode(
    filename = isModule ? "app.module.js" : isUmd ? "app.umd.js" : "app.js"
  ) {
    const me = this;
    ...
    if (!code) {
      try {
        const response = await AjaxHelper.get(
          location.href.replace(/[^/]*$/, "") + filename
        );
  ...
  updateDownloadLink() {
    ...
    downloadLink.href = `data:text/${
      this.filename.endsWith(".css") ? "css" : "javascript"
    };charset=utf-8,${escape(this.codeElement.innerText)}`;
  }
  ...
  toggleReadOnly() {
    const me = this,
      ...
      readOnly =
        me.mode === "js" && (me.hasImports || isUmd || !me.supportsImport);
    ...
    if (readOnly) {
      ...
      me.status =
        '<i class="b-fa b-fa-lock" /> Read only' +
        (!me.supportsImport ? " (try it on Chrome or Firefox)" : "");
    } else {
    ...
    contentElement
      .querySelector("code")
      .setAttribute("contenteditable", !readOnly);
  }
...
setTimeout(async () => {
  ...
  window.scheduler =
    bryntum.query("scheduler", true) || bryntum.query("schedulerpro", true);
  ...
  if (
    (window.grid || window.scheduler || window.gantt || window.calendar) &&
    !isDemoBrowser
  ) {
