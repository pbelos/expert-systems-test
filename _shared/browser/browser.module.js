...
class ExamplesApp {
  constructor() {
    const me = this,
      ...
      examples = (window.examples || []).map((example) =>
        Object.assign({ fullFolder: this.exampleFolder(example) }, example)
      ),
      storageName = (name) => `bryntum-${shared.productName}-demo-${name}`,
      saveToStorage = (name, value) =>
        BrowserHelper.setLocalStorageItem(storageName(name), value),
      loadFromStorage = (name) =>
        BrowserHelper.getLocalStorageItem(storageName(name)),
    ...
    document.getElementById(
      "title"
    ).innerHTML = `${shared.productFullName} ${version}`;
    ...
    me.isOnline =
      /^(www\.)?bryntum\.com/.test(location.host) ||
      location.search.includes("online");
    me.buildTip = me.isOnline
      ? "This demo is not viewable online, but included when you download the trial. "
      : "This demo needs to be built before it can be viewed. ";
    ...
    const [toolbar] = WidgetHelper.append(
      {
        ...
        items: {
          jumpTo: {
            ...
            editable: false,
            placeholder: "Jump to",
            items: store.groupRecords.map((r) => ({
              id: r.id,
          ...
          filterField: {
            ...
            filterFunction(record, value) {
              ...
              return value
                ?.toLowerCase()
                .split(" ")
                .every((word) => record.title.toLowerCase().includes(word));
            },
          ...
          upgradeButton: {
            ...
            href: me.isOnline
              ? `/docs/${
                  shared.productName === "schedulerpro"
                    ? "schedulerpro-pro"
                    : shared.productName
                }#upgrade-guide`
              : "../docs#upgrade-guide",
  ...
  scrollToDemo(demo, options = { behavior: "smooth" }) {
    ...
    if (demo) {
      const element = document.getElementById(
        "b-example-" + demo.folder.replace(/\//g, "-")
      );
      if (BrowserHelper.isIE11) {
  ...
  scrollToGroup(group, options = { behavior: "smooth" }) {
    this.scrollToDemo(
      this.exampleStore.find((example) => example.group === group),
      options
    );
  }
  ...
  getDomConfig() {
    const { theme } = shared,
      version = VersionHelper.getVersion(shared.productName),
      isNew = (example) =>
        version && example.since && version.startsWith(example.since),
      isUpdated = (example) =>
        version && example.updated && version.startsWith(example.updated),
      configs = [];
  ...
  render() {
    ...
    Widget.attachTooltip(me.examplesContainerEl, {
      ...
      getHtml: async ({ tip }) => {
        ...
        const url = `${
          linkNode.getAttribute("href") || linkNode.dataset.linkUrl
        }/app.config.json`;
        ...
        if (!me.currentTipLoadPromiseByURL[url]) {
          const requestPromise = (me.currentTipLoadPromiseByURL[
              url
            ] = AjaxHelper.get(url, { parseJson: true })),
            response = await requestPromise,
            json = response.parsedJson,
            html = (activeTarget.dataset.tooltip =
              json.description.replace(/[\n\r]/g, "") +
              (/build/.test(activeTarget.className)
                ? `<br><b>${me.buildTip}</b>`
                : ""));
    ...
    document
      ...
      .addEventListener("click", me.onCloseClick.bind(me));
    ...
    EventHelper.on({
      ...
      click(event) {
        ...
        new Popup({
          ...
          header:
            '<i class="b-fa b-fa-cog"></i> ' +
            (me.isOnline ? "Download needed" : "Needs building"),
          html:
            me.buildTip +
            `The demo can be found in distribution folder: <i class="b-fa b-fa-folder-open"> <b>` +
            (!me.isOnline
              ? `<a href="${el.dataset.linkUrl}">${el.dataset.linkText}</a>`
              : el.dataset.linkText) +
            "</b>",
          closeAction: "destroy",
          textContent: false,
  ...
  embedDescriptions() {
    return new Promise((resolve) => {
      ...
      this.examplesStore.forEach((example) => {
        promises.push(
          AjaxHelper.get(this.exampleConfig(example), { parseJson: true }).then(
            (response) => {
  ...
  exampleId(example) {
    return `b-example-${example.fullFolder
      .replace(/\.\.\//gm, "")
      .replace(/\//gm, "-")}`;
  }

  exampleLinkText(example) {
    return this.exampleFolder(example, "examples/")
      .replace(/\.\.\//gm, "")
      .replace(/\//gm, "/<wbr>");
  }
